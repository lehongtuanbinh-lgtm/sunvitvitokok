...
// --- HỆ THỐNG CHỐNG CRASH SERVER (BẤT TỬ) ---
process.on('uncaughtException', (err) => {
  console.error('[Anti-Crash] Phát hiện lỗi chưa xử lý:', err.message);
});
process.on('unhandledRejection', (reason, promise) => {
  console.error('[Anti-Crash] Lỗi Promise chưa xử lý:', reason);
});
...
// FIX & NÂNG CẤP: Lưu file bất đồng bộ (Tránh đơ event loop của server)
let isSavingLearningData = false;
function saveLearningData() {
  if (isSavingLearningData) return;
  isSavingLearningData = true;
  try {
    fs.writeFile(LEARNING_FILE, JSON.stringify(learningData, null, 2), (err) => {
      isSavingLearningData = false;
      if (err) console.error('Error saving learning data:', err.message);
    });
  } catch (error) {
    isSavingLearningData = false;
    console.error('Error in saveLearningData:', error.message);
  }
}
...
// FIX & NÂNG CẤP: Lưu file bất đồng bộ chống đơ lag
...
function savePredictionHistory() {
  ...
  try {
    ...
    fs.writeFile(HISTORY_FILE, JSON.stringify(dataToSave, null, 2), (err) => {
      isSavingHistory = false;
      if (err) console.error('Error saving prediction history:', err.message);
    });
  } catch (error) {
    isSavingHistory = false;
...
// FIX & NÂNG CẤP: Hệ thống Cache & Timeout giúp API chạy nhanh, không bị treo do bên thứ 3
let cacheData = null;
let lastFetchTime = 0;
...
async function fetchData() {
  const now = Date.now();
  // Trả về cache nếu dữ liệu mới lấy trong 1.5 giây qua (Chống spam API)
  if (cacheData && (now - lastFetchTime < 1500)) {
    return cacheData;
  }
  
  // Nếu có request khác đang gọi, đợi 50ms rồi trả về cache để không đơ luồng
  if (isFetchingAPI) {
  ...
  isFetchingAPI = true;
  try {
    // Thêm Timeout 4000ms để không bị treo server
    ...
    if (rawData && rawData.lich_su_phien) {
      ...
      const formattedData = { data: [currentPhien, ...historyData] };
    ...
    console.error('[Anti-Lag] Lỗi lấy dữ liệu API gốc:', error.message);
    // Nhả lại cache cũ để server vẫn phản hồi mượt mà không crash
    return cacheData; 
  } finally {
    isFetchingAPI = false;
  }
...
// --- GIỮ NGUYÊN TOÀN BỘ CÁC HÀM ANALYZE ---
...
function analyzeCau22(results, type) {
  ...
  while (i < results.length - 1 && pairCount < 4) {
    if (results[i] === results[i + 1]) {
      pattern.push(results[i]); pairCount++; i += 2;
    } else { break; }
  }
  if (pairCount >= 2) {
...
function analyzeCau121(results, type) {
  ...
  if (pattern1[0] !== pattern1[1] && pattern1[1] === pattern1[2] && 
      pattern1[2] !== pattern1[3] && pattern1[0] === pattern1[3]) {
    const weight = getPatternWeight(type, 'cau_121');
    return { 
      detected: true, pattern: '1-2-1', prediction: pattern1[0],
      confidence: Math.round(10 * weight), name: 'Cầu 1-2-1', patternId: 'cau_121'
    };
  }
  return { detected: false };
}
...
function analyzeCau123(results, type) {
  ...
  if (nextTwo[0] === nextTwo[1] && nextTwo[0] !== first) {
    ...
    if (allSame && lastThree[0] !== nextTwo[0]) {
      ...
      return { 
        detected: true, pattern: '1-2-3', prediction: first,
        confidence: Math.round(11 * weight), name: 'Cầu 1-2-3', patternId: 'cau_123'
      };
    }
  }
  return { detected: false };
...
function analyzeCau321(results, type) {
  ...
  if (first3Same && next2Same && first3[0] !== next2[0] && last1 !== next2[0]) {
    ...
    return { 
      detected: true, pattern: '3-2-1', prediction: next2[0],
      confidence: Math.round(12 * weight), name: 'Cầu 3-2-1', patternId: 'cau_321'
    };
  }
  return { detected: false };
}
...
function analyzeCauNhayCoc(results, type) {
  ...
  if (skipPattern.length >= 3) {
    ...
    if (allSame) {
      return { 
        detected: true, pattern: skipPattern.slice(0, 3), prediction: skipPattern[0],
        confidence: Math.round(8 * weight), name: 'Cầu Nhảy Cóc', patternId: 'cau_nhay_coc'
      };
    }
    let alternating = true;
    for (let i = 1; i < skipPattern.length - 1; i++) {
      if (skipPattern[i] === skipPattern[i - 1]) { alternating = false; break; }
    }
    if (alternating && skipPattern.length >= 3) {
      return { 
        detected: true, pattern: skipPattern.slice(0, 3), prediction: skipPattern[0] === 'Tài' ? 'Xỉu' : 'Tài',
        confidence: Math.round(7 * weight), name: 'Cầu Nhảy Cóc Đảo', patternId: 'cau_nhay_coc'
      };
    }
  }
  return { detected: false };
...
function analyzeCauNhipNghieng(results, type) {
  ...
  if (taiCount5 >= 4) {
    return { 
      detected: true, type: 'nghieng_5', ratio: `${taiCount5}/5 Tài`, prediction: 'Tài',
      confidence: Math.round(9 * weight), name: `Cầu Nhịp Nghiêng 5 (${taiCount5} Tài)`, patternId: 'cau_nhip_nghieng'
    };
  } else if (taiCount5 <= 1) {
    return { 
      detected: true, type: 'nghieng_5', ratio: `${5 - taiCount5}/5 Xỉu`, prediction: 'Xỉu',
      confidence: Math.round(9 * weight), name: `Cầu Nhịp Nghiêng 5 (${5 - taiCount5} Xỉu)`, patternId: 'cau_nhip_nghieng'
    };
  }
  ...
  if (results.length >= 7) {
    ...
    if (taiCount7 >= 5) {
      return { 
        detected: true, type: 'nghieng_7', ratio: `${taiCount7}/7 Tài`, prediction: 'Tài',
        confidence: Math.round(10 * weight), name: `Cầu Nhịp Nghiêng 7 (${taiCount7} Tài)`, patternId: 'cau_nhip_nghieng'
      };
    } else if (taiCount7 <= 2) {
      return { 
        detected: true, type: 'nghieng_7', ratio: `${7 - taiCount7}/7 Xỉu`, prediction: 'Xỉu',
        confidence: Math.round(10 * weight), name: `Cầu Nhịp Nghiêng 7 (${7 - taiCount7} Xỉu)`, patternId: 'cau_nhip_nghieng'
      };
    }
  }
  return { detected: false };
...
function analyzeCau3Van1(results, type) {
  ...
  if (taiCount === 3) {
    ...
    if (xiuIndex === 3) {
      ...
      return { 
        detected: true, pattern: '3-1', prediction: 'Tài',
        confidence: Math.round(8 * weight), name: 'Cầu 3 Ván 1 (3T-1X)', patternId: 'cau_3van1'
      };
    }
  } else if (taiCount === 1) {
    ...
    if (taiIndex === 3) {
      const weight = getPatternWeight(type, 'cau_3van1');
      return { 
        detected: true, pattern: '3-1', prediction: 'Xỉu',
        confidence: Math.round(8 * weight), name: 'Cầu 3 Ván 1 (3X-1T)', patternId: 'cau_3van1'
      };
    }
  }
  return { detected: false };
...
function analyzeCauBeCau(results, type) {
  ...
  if (streakLength >= 4) {
    ...
    return { 
      detected: true, streakLength, prediction: results[0] === 'Tài' ? 'Xỉu' : 'Tài',
      confidence: Math.round(Math.min(14, streakLength * 2 + 4) * weight),
      name: `Cầu Bẻ Cầu (${streakLength} phiên ${results[0]})`, patternId: 'cau_be_cau'
    };
  }
  return { detected: false };
}
...
function analyzeCauTuNhien(results, type) {
  ...
  return {
    detected: true, prediction, confidence: Math.round(5 * weight),
    name: `Cầu Tự Nhiên (${taiCount}T-${xiuCount}X)`, patternId: 'cau_tu_nhien'
  };
}

function analyzeCau44(results, type) {
  if (results.length < 8) return { detected: false };
  let quadCount = 0; let i = 0; let pattern = [];
  ...
  if (quadCount >= 1) {
    ...
    let prediction;
    if (currentPosition >= 3) { prediction = lastQuadType === 'Tài' ? 'Xỉu' : 'Tài'; } 
    else { prediction = lastQuadType; }
    return { 
      detected: true, quadCount, prediction,
      confidence: Math.round(Math.min(14, quadCount * 4 + 6) * weight),
      name: `Cầu 4-4 (${quadCount} bộ bốn)`, patternId: 'cau_44'
    };
  }
  return { detected: false };
}
...
function analyzeCau55(results, type) {
  ...
  let quintCount = 0; let i = 0; let pattern = [];
  ...
  if (quintCount >= 1) {
    ...
    let prediction;
    if (currentPosition >= 4) { prediction = lastQuintType === 'Tài' ? 'Xỉu' : 'Tài'; } 
    else { prediction = lastQuintType; }
    return { 
      detected: true, quintCount, prediction,
      confidence: Math.round(Math.min(15, quintCount * 5 + 7) * weight),
      name: `Cầu 5-5 (${quintCount} bộ năm)`, patternId: 'cau_55'
    };
  }
  return { detected: false };
}
...
function analyzeCau212(results, type) {
  if (results.length < 5) return { detected: false };
  const pattern = results.slice(0, 5); const weight = getPatternWeight(type, 'cau_212');
  if (pattern[0] === pattern[1] && pattern[1] !== pattern[2] && pattern[2] === pattern[3] && 
      pattern[3] !== pattern[4] && pattern[0] !== pattern[2]) {
    return { 
      detected: true, pattern: '2-1-2', prediction: pattern[0] === 'Tài' ? 'Xỉu' : 'Tài',
      confidence: Math.round(10 * weight), name: 'Cầu 2-1-2', patternId: 'cau_212'
    };
  }
  return { detected: false };
}
...
function analyzeCau1221(results, type) {
  if (results.length < 6) return { detected: false };
  const pattern = results.slice(0, 6); const weight = getPatternWeight(type, 'cau_1221');
  if (pattern[0] !== pattern[1] && pattern[1] === pattern[2] && pattern[2] === pattern[3] && 
      pattern[3] !== pattern[4] && pattern[4] === pattern[5] && pattern[0] !== pattern[1]) {
    return { 
      detected: true, pattern: '1-2-2-1', prediction: pattern[0],
      confidence: Math.round(11 * weight), name: 'Cầu 1-2-2-1', patternId: 'cau_1221'
    };
  }
  return { detected: false };
}
...
function analyzeCau2112(results, type) {
  if (results.length < 6) return { detected: false };
  const pattern = results.slice(0, 6); const weight = getPatternWeight(type, 'cau_2112');
  if (pattern[0] === pattern[1] && pattern[1] !== pattern[2] && pattern[2] === pattern[3] && 
      pattern[3] !== pattern[4] && pattern[4] === pattern[5] && pattern[0] !== pattern[2]) {
    return { 
      detected: true, pattern: '2-1-1-2', prediction: pattern[0],
      confidence: Math.round(11 * weight), name: 'Cầu 2-1-1-2', patternId: 'cau_2112'
    };
  }
  return { detected: false };
}
...
function analyzeCauGap(results, type) {
  ...
  for (let gapSize = 2; gapSize <= 3; gapSize++) {
    ...
    for (let i = 0; i < Math.min(results.length, 12); i += (gapSize + 1)) {
      if (results[i] !== referenceType) { patternFound = false; break; }
    }
    if (patternFound) {
      return { 
        detected: true, gapSize, prediction: referenceType,
        confidence: Math.round(9 * weight), name: `Cầu Gấp ${gapSize + 1} (mỗi ${gapSize + 1} phiên)`, patternId: 'cau_gap'
      };
    }
  }
  return { detected: false };
...
function analyzeCauZiczac(results, type) {
  ...
  if (zigzagCount >= 3) {
    return { 
      detected: true, zigzagCount, prediction: results[0] === 'Tài' ? 'Xỉu' : 'Tài',
...
function analyzeCauDoi(results, type) {
  ...
  let pairChanges = 0; let i = 0;
  while (i < results.length - 1) {
    if (results[i] === results[i + 1]) { pairChanges++; i += 2; } else { break; }
  }
  if (pairChanges >= 2) {
    const isAlternatingPairs = results[0] !== results[2];
    if (isAlternatingPairs) {
      return { 
        detected: true, pairChanges, prediction: results[0] === 'Tài' ? 'Xỉu' : 'Tài',
        confidence: Math.round(Math.min(12, pairChanges * 3 + 4) * weight),
        name: `Cầu Đôi Đảo (${pairChanges} cặp)`, patternId: 'cau_doi'
      };
    } else {
      return { 
        detected: true, pairChanges, prediction: results[0],
        confidence: Math.round(Math.min(11, pairChanges * 2 + 5) * weight),
        name: `Cầu Đôi Bệt (${pairChanges} cặp)`, patternId: 'cau_doi'
      };
    }
  }
  return { detected: false };
...
function analyzeSmartBet(results, type) {
  ...
  if (trendChanging) {
    ...
    return { 
      detected: true, trendChange: true, prediction: currentDominant === 'Tài' ? 'Xỉu' : 'Tài',
      confidence: Math.round(13 * weight),
      name: `Đảo Xu Hướng (${taiLast5}T-${5-taiLast5}X → ${taiPrev5}T-${5-taiPrev5}X)`, patternId: 'smart_bet'
    };
  }
  ...
  if (taiLast10 >= 8 || taiLast10 <= 2) {
    ...
    return { 
      detected: true, extreme: true, prediction: dominant === 'Tài' ? 'Xỉu' : 'Tài',
...
function analyzeDistribution(data, type, windowSize = 50) {
  ...
  return {
    taiPercent: (taiCount / window.length) * 100, xiuPercent: (xiuCount / window.length) * 100,
    taiCount, xiuCount, total: window.length, imbalance: Math.abs(taiCount - xiuCount) / window.length
  };
}

function analyzeDicePatterns(data) {
  const recentData = data.slice(0, 15);
  let highDiceCount = 0; let lowDiceCount = 0; let totalSum = 0; let sumVariance = [];
  recentData.forEach(d => {
    const dices = [d.Xuc_xac_1, d.Xuc_xac_2, d.Xuc_xac_3];
    dices.forEach(dice => { if (dice >= 4) highDiceCount++; else lowDiceCount++; });
    totalSum += d.Tong; sumVariance.push(d.Tong);
  });
  ...
  return {
    ...
    averageSum: avgSum, standardDeviation: stdDev, sumTrend: avgSum > 10.5 ? 'high' : 'low', isStable: stdDev < 3
  };
}

function analyzeSumTrend(data) {
  const recentSums = data.slice(0, 20).map(d => d.Tong);
  let increasingCount = 0; let decreasingCount = 0;
  ...
  return {
    ...
    movingAvg5, movingAvg10, shortTermBias: movingAvg5 > 10.5 ? 'Tài' : 'Xỉu'
  };
}

function analyzeRecentMomentum(results) {
  const windows = [3, 5, 10, 15]; const momentum = {};
  windows.forEach(size => {
    if (results.length >= size) {
      ...
      momentum[`window_${size}`] = {
        taiRatio: taiCount / size, xiuRatio: (size - taiCount) / size, dominant: taiCount > size / 2 ? 'Tài' : 'Xỉu'
      };
    }
  });
  return momentum;
...
function detectCyclePattern(results, type) {
  ...
  for (let cycleLength = 2; cycleLength <= 6; cycleLength++) {
    ...
    if (isRepeating) {
      ...
      return { 
        detected: true, cycleLength, pattern, prediction: pattern[nextPosition],
...
function analyzeEdgeCases(data, type) {
  ...
  if (extremeHighCount >= 4) {
    return { 
      detected: true, type: 'extreme_high', prediction: 'Xỉu', confidence: Math.round(7 * weight),
      name: `Cực Điểm Cao (${extremeHighCount} phiên >= 14)`, patternId: 'edge_cases'
    };
  }
  if (extremeLowCount >= 4) {
    return { 
      detected: true, type: 'extreme_low', prediction: 'Tài', confidence: Math.round(7 * weight),
...
function analyzeDiceTrendLine(data, type) {
  ...
  const current = data[0]; const previous = data[1];
  ...
  const previousResult = previous.Ket_qua; const weight = getPatternWeight(type, 'dice_trend_line');
  
  const allSameDice = currentDices[0] === currentDices[1] && currentDices[1] === currentDices[2];
  if (allSameDice) {
    const prediction = currentDices[0] >= 4 ? 'Xỉu' : 'Tài';
    return {
      detected: true, type: 'same_dice', prediction, confidence: Math.round(13 * weight),
      name: `Biểu Đồ Đường (3 xúc xắc giống ${currentDices[0]})`, patternId: 'dice_trend_line',
  ...
  if (twoSameDice) {
    ...
    return {
      detected: true, type: 'two_same_dice', prediction, confidence: Math.round(11 * weight),
      name: `Biểu Đồ Đường (2 xúc xắc giống - Bẻ ${previousResult})`, patternId: 'dice_trend_line',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  
  const maxDice = Math.max(...currentDices); const minDice = Math.min(...currentDices);
  if (maxDice === 6 && minDice === 1) {
    const prediction = previousResult === 'Tài' ? 'Xỉu' : 'Tài';
    return {
      detected: true, type: 'extreme_range', prediction, confidence: Math.round(12 * weight),
      name: `Biểu Đồ Đường (Biên độ max 6-1 - Bẻ)`, patternId: 'dice_trend_line',
  ...
  if (upCount === 1 && downCount === 2) {
    return {
      detected: true, type: 'trend_1up_2down', prediction: 'Tài', confidence: Math.round(12 * weight),
      name: `Biểu Đồ Đường (1 lên 2 xuống → Tài)`, patternId: 'dice_trend_line',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  if (upCount === 2 && downCount === 1) {
    return {
      detected: true, type: 'trend_2up_1down', prediction: 'Xỉu', confidence: Math.round(12 * weight),
      name: `Biểu Đồ Đường (2 lên 1 xuống → Xỉu)`, patternId: 'dice_trend_line',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  if (upCount === 3 || downCount === 3) {
    ...
    return {
      detected: true, type: 'all_same_direction', prediction, confidence: Math.round(10 * weight),
      name: `Biểu Đồ Đường (3 dây cùng ${upCount === 3 ? 'lên' : 'xuống'} → Theo ${previousResult})`, patternId: 'dice_trend_line',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  ...
  if (twoSameDirection) {
    ...
    return {
      detected: true, type: 'two_same_direction', prediction, confidence: Math.round(10 * weight),
      name: `Biểu Đồ Đường (${directionDesc} → Bẻ ${previousResult})`, patternId: 'dice_trend_line',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  return { detected: false };
...
function analyzeDayGay(data, type) {
  ...
  if (sameCount === 2 && upCount === 1) {
    ...
    if (sameDiceValues[0] === sameDiceValues[1]) {
      ...
      return {
        detected: true, type: 'day_gay_2thang_1len', prediction: 'Xỉu', confidence: Math.round(14 * weight),
        name: `Dây Gãy (2 dây thẳng ${sameValueDesc} + 1 lên → Xỉu)`, patternId: 'day_gay',
  ...
  if (sameCount === 2 && downCount === 1) {
    ...
    if (sameDiceValues[0] === sameDiceValues[1]) {
      const sameValueDesc = `${sameDiceValues[0]}-${sameDiceValues[1]}`;
      return {
        detected: true, type: 'day_gay_2thang_1xuong', prediction: 'Tài', confidence: Math.round(14 * weight),
        name: `Dây Gãy (2 dây thẳng ${sameValueDesc} + 1 xuống → Tài)`, patternId: 'day_gay',
        analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions, sameDiceValues }
      };
    }
  }
  if (upCount === 2 && downCount === 1) {
    return {
      detected: true, type: 'day_gay_2len_1xuong', prediction: 'Xỉu', confidence: Math.round(13 * weight),
      name: `Dây Gãy (2 lên 1 xuống → Xỉu)`, patternId: 'day_gay',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  if (downCount === 2 && upCount === 1) {
    return {
      detected: true, type: 'day_gay_2xuong_1len', prediction: 'Tài', confidence: Math.round(13 * weight),
      name: `Dây Gãy (2 xuống 1 lên → Tài)`, patternId: 'day_gay',
      analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions }
    };
  }
  return { detected: false };
...
function analyzeBreakPattern(results, data, type) {
  ...
  if (streakLength >= 5) {
    const current = data[0]; const previous = data[1];
    const currentSum = current.Tong; const previousSum = previous.Tong;
    const sumDiff = Math.abs(currentSum - previousSum);
    if (sumDiff >= 5) {
      const prediction = results[0] === 'Tài' ? 'Xỉu' : 'Tài';
      return {
        detected: true, type: 'break_after_streak', prediction, confidence: Math.round(15 * weight),
        name: `Cầu Liên Tục ${streakLength} (Biến động ${sumDiff} → Bẻ)`, patternId: 'break_pattern',
    ...
    if (streakLength >= 7) {
      const prediction = results[0] === 'Tài' ? 'Xỉu' : 'Tài';
      return {
        detected: true, type: 'long_streak_break', prediction, confidence: Math.round(16 * weight),
        name: `Cầu Liên Tục ${streakLength} (Streak dài → Bẻ mạnh)`, patternId: 'break_pattern',
...
function analyzeFibonacciPattern(data, type) {
  ...
  let taiAtFib = 0; let xiuAtFib = 0;
  fibPositions.forEach(pos => {
    if (pos <= data.length) {
      const result = data[pos - 1].Ket_qua;
      if (result === 'Tài') taiAtFib++; else xiuAtFib++;
    }
  });
  if (taiAtFib >= 5 || xiuAtFib >= 5) {
    const dominant = taiAtFib > xiuAtFib ? 'Tài' : 'Xỉu';
    return {
      detected: true, type: 'fibonacci_dominant', prediction: dominant, confidence: Math.round(11 * weight),
...
function analyzeMomentumPattern(data, type) {
  ...
  if (Math.abs(momentumChange) >= 2) {
    ...
    return {
      detected: true, type: 'momentum_shift', prediction, confidence: Math.round(12 * weight),
      name: `Momentum ${momentumChange > 0 ? 'Tăng' : 'Giảm'} (${avgLast5.toFixed(1)} vs ${avgPrev5.toFixed(1)})`, patternId: 'momentum',
...
function analyzeResistanceSupport(data, type) {
  ...
  const maxSum = Math.max(...sums); const minSum = Math.min(...sums); const currentSum = data[0].Tong;
  const resistance = maxSum; const support = minSum;
  const distToResistance = resistance - currentSum; const distToSupport = currentSum - support;
  
  if (distToResistance <= 2 && distToResistance < distToSupport) {
    return {
      detected: true, type: 'near_resistance', prediction: 'Xỉu', confidence: Math.round(10 * weight),
      name: `Gần Kháng Cự (${currentSum} → ${resistance})`, patternId: 'resistance_support',
      analysis: { currentSum, resistance, distToResistance }
    };
  }
  if (distToSupport <= 2 && distToSupport < distToResistance) {
    return {
      detected: true, type: 'near_support', prediction: 'Tài', confidence: Math.round(10 * weight),
      name: `Gần Hỗ Trợ (${currentSum} → ${support})`, patternId: 'resistance_support',
...
function analyzeWavePattern(data, type) {
  ...
  let waves = []; let currentWave = { type: results[0], count: 1 };
  for (let i = 1; i < results.length; i++) {
    if (results[i] === currentWave.type) { currentWave.count++; } 
    else { waves.push(currentWave); currentWave = { type: results[i], count: 1 }; }
  }
  ...
  if (waves.length >= 4) {
    ...
    if (isIncreasing && waveLengths[0] < waveLengths[3]) {
      ...
      return {
        detected: true, type: 'wave_expanding', prediction, confidence: Math.round(12 * weight),
        name: `Sóng Mở Rộng (${waveLengths.join('-')} → Bẻ ${prediction})`, patternId: 'wave',
    ...
    if (isDecreasing && waveLengths[0] > waveLengths[3]) {
      ...
      return {
        detected: true, type: 'wave_contracting', prediction, confidence: Math.round(11 * weight),
        name: `Sóng Thu Hẹp (${waveLengths.join('-')} → Theo ${prediction})`, patternId: 'wave',
  ...
  if (waves.length >= 3) {
    ...
    if (waves[0].count > avgWaveLength * 1.5) {
      const prediction = waves[0].type === 'Tài' ? 'Xỉu' : 'Tài';
      return {
        detected: true, type: 'wave_peak', prediction, confidence: Math.round(11 * weight),
        name: `Đỉnh Sóng (${waves[0].count} > avg ${avgWaveLength.toFixed(1)} → Bẻ)`, patternId: 'wave',
...
function analyzeGoldenRatio(data, type) {
  ...
  let taiAtGolden = 0; let xiuAtGolden = 0;
  goldenPositions.forEach(pos => {
    if (pos <= results.length) {
      const result = results[pos - 1].Ket_qua;
      if (result === 'Tài') taiAtGolden++; else xiuAtGolden++;
    }
  });
  ...
  if (ratio >= 1.6 && ratio <= 1.7) {
    ...
    return {
      detected: true, type: 'golden_ratio_detected', prediction: dominant, confidence: Math.round(12 * weight),
      name: `Tỷ Lệ Vàng (${taiAtGolden}T:${xiuAtGolden}X = ${ratio.toFixed(2)} → ${dominant})`, patternId: 'golden_ratio',
  ...
  if (taiAtGolden >= 5 || xiuAtGolden >= 5) {
    ...
    return {
      detected: true, type: 'golden_extreme', prediction, confidence: Math.round(11 * weight),
      name: `Fibonacci Cực (${Math.max(taiAtGolden, xiuAtGolden)}/7 → Bẻ ${prediction})`, patternId: 'golden_ratio',
...
function analyzeMarkovChain(results, data, type) {
  ...
  for (let i = 0; i < results.length - 1; i++) {
    const from = results[i + 1]; const to = results[i];
    const key = `${from}->${to}`; transitions[key]++;
  }
  ...
  const taiToTai = transitions['Tài->Tài']; const taiToXiu = transitions['Tài->Xỉu'];
  const xiuToTai = transitions['Xỉu->Tài']; const xiuToXiu = transitions['Xỉu->Xỉu'];
  ...
  if (Math.abs(probability - 0.5) > 0.1) {
    return {
      ...
      name: `Markov Chain (${currentResult} → ${prediction}: ${(probability * 100).toFixed(0)}%)`, patternId: 'markov_chain',
...
function analyzeMovingAverageDrift(data, type) {
  ...
  const shortTermDrift = ma5 - ma10; const longTermDrift = ma10 - ma20; const totalDrift = ma5 - ma20;
  ...
  if (Math.abs(shortTermDrift) > 1.5 && Math.abs(longTermDrift) > 1) {
    ...
    if (shortTermDrift * longTermDrift > 0) {
      return {
        detected: true, type: 'strong_drift', prediction, confidence: Math.round(14 * weight),
        name: `MA Drift Mạnh (MA5:${ma5.toFixed(1)} MA10:${ma10.toFixed(1)} → ${momentum})`, patternId: 'moving_avg_drift',
  ...
  if (Math.abs(totalDrift) > 2) {
    ...
    return {
      detected: true, type: 'reversal_drift', prediction, confidence: Math.round(12 * weight),
      name: `MA Đảo Chiều (Drift: ${totalDrift.toFixed(1)} → Bẻ ${prediction})`, patternId: 'moving_avg_drift',
  ...
  if (Math.abs(ema5 - ma10) > 1.5) {
    ...
    return {
      detected: true, type: 'ema_crossover', prediction, confidence: Math.round(11 * weight),
      name: `EMA Crossover (EMA5:${ema5.toFixed(1)} vs MA10:${ma10.toFixed(1)})`, patternId: 'moving_avg_drift',
...
function analyzeSumPressure(data, type) {
  ...
  if (Math.abs(deviation) > 1.5) {
    ...
    return {
      ...
      name: `Áp Lực Tổng ${pressure.toUpperCase()} (Avg:${avgSum.toFixed(1)} vs Mean:${EXPECTED_MEAN})`, patternId: 'sum_pressure',
      analysis: { avgSum, deviation, expectedMean: EXPECTED_MEAN }
    };
  }
  if (extremeHighCount >= 4) {
    return {
      detected: true, type: 'extreme_high_pressure', prediction: 'Xỉu', confidence: Math.round(13 * weight),
      name: `Áp Lực Cực Cao (${extremeHighCount}/15 phiên >= 14)`, patternId: 'sum_pressure',
      analysis: { extremeHighCount, recentSums }
    };
  }
  if (extremeLowCount >= 4) {
    return {
      detected: true, type: 'extreme_low_pressure', prediction: 'Tài', confidence: Math.round(13 * weight),
      name: `Áp Lực Cực Thấp (${extremeLowCount}/15 phiên <= 7)`, patternId: 'sum_pressure',
  ...
  if (stdDev < 2 && normalCount >= 10) {
    ...
    return {
      detected: true, type: 'stable_zone', prediction, confidence: Math.round(10 * weight),
      name: `Vùng Ổn Định (StdDev:${stdDev.toFixed(1)}, Last:${lastSum})`, patternId: 'sum_pressure',
...
function analyzeVolatility(data, type) {
  ...
  const maxChange = Math.max(...changes); const recentChange = changes[0];
  ...
  if (avgChange > 4 && maxChange >= 7) {
    ...
    return {
      detected: true, type: 'high_volatility', prediction, confidence: Math.round(12 * weight),
      name: `Biến Động Cao (Avg:${avgChange.toFixed(1)}, Max:${maxChange} → Bẻ)`, patternId: 'volatility',
  ...
  if (avgChange < 2 && recentChange >= 5) {
    const lastResult = data[0].Ket_qua;
    return {
      detected: true, type: 'volatility_spike', prediction: lastResult, confidence: Math.round(11 * weight),
      name: `Đột Biến Biến Động (Spike:${recentChange} vs Avg:${avgChange.toFixed(1)})`, patternId: 'volatility',
...
function analyzeSunHotCold(results, last50, type) {
  if (results.length < 10) return { detected: false };
  const last10 = results.slice(0, 10); const last20 = results.slice(0, Math.min(20, results.length));
  const taiCount10 = last10.filter(r => r === 'Tài').length; const xiuCount10 = 10 - taiCount10;
  const taiCount20 = last20.filter(r => r === 'Tài').length; const xiuCount20 = last20.length - taiCount20;
  const weight = getPatternWeight(type, 'sun_hot_cold');
  
  if (taiCount10 >= 7) {
    const hotStrength = taiCount10 / 10;
    return {
      detected: true, type: 'hot_tai', prediction: taiCount10 >= 8 ? 'Xỉu' : 'Tài',
      confidence: Math.round((taiCount10 >= 8 ? 14 : 12) * weight * hotStrength),
      name: `Sun Nóng Tài (${taiCount10}/10)`, patternId: 'sun_hot_cold'
    };
  } else if (xiuCount10 >= 7) {
    const hotStrength = xiuCount10 / 10;
    return {
      detected: true, type: 'hot_xiu', prediction: xiuCount10 >= 8 ? 'Tài' : 'Xỉu',
      confidence: Math.round((xiuCount10 >= 8 ? 14 : 12) * weight * hotStrength),
      name: `Sun Nóng Xỉu (${xiuCount10}/10)`, patternId: 'sun_hot_cold'
    };
  }
  ...
  if (ratio20 >= 0.7 || ratio20 <= 0.3) {
    ...
    return {
      detected: true, type: 'trend_20', prediction: dominant, confidence: Math.round(10 * weight),
      name: `Xu Hướng 20 Phiên (${dominant})`, patternId: 'sun_hot_cold'
    };
  }
  return { detected: false };
}
...
function analyzeSunStreakBreak(results, last50, type) {
  if (results.length < 5) return { detected: false };
  let currentStreak = 1; const currentType = results[0];
  for (let i = 1; i < results.length; i++) {
    if (results[i] === currentType) { currentStreak++; } else { break; }
  }
  ...
  if (currentStreak >= 4) {
    ...
    return {
      detected: true, streakLength: currentStreak, streakType: currentType,
      ...
      name: `Sun Streak ${currentStreak} ${currentType} ${shouldBreak ? '(Bẻ)' : '(Tiếp)'}`, patternId: 'sun_streak_break'
    };
  }
  return { detected: false };
}
...
function analyzeSunBalance(results, type) {
  ...
  if (diff >= 7) {
    ...
    return {
      detected: true, type: 'imbalance', prediction: minority, confidence: Math.round(Math.min(13, diff + 5) * weight),
      name: `Sun Cân Bằng (T:${taiCount} - X:${xiuCount})`, patternId: 'sun_balance'
    };
  }
  if (diff <= 1) {
    ...
    return {
      detected: true, type: 'balanced', prediction, confidence: Math.round(8 * weight),
      name: `Sun Cân Bằng Hoàn Hảo`, patternId: 'sun_balance'
    };
  }
  return { detected: false };
}
...
function analyzeSunMomentumShift(results, last50, type) {
  if (results.length < 12) return { detected: false };
  const recent6 = results.slice(0, 6); const prev6 = results.slice(6, 12);
  ...
  if (Math.abs(shift) >= 4) {
    ...
    return {
      detected: true, type: 'momentum_shift', prediction: shifting,
      confidence: Math.round(Math.min(14, Math.abs(shift) * 2 + 4) * weight),
      name: `Sun Đổi Chiều → ${shifting}`, patternId: 'sun_momentum_shift'
    };
  }
  return { detected: false };
}
...
function applyAutoReversal(type, prediction) {
  ...
  if (!reversalState) {
    learningData[type].reversalState = {
      active: false, activatedAt: null, consecutiveLosses: 0, reversalCount: 0, lastReversalResult: null
    };
    return { prediction, reversed: false };
  }
  if (streakAnalysis.currentStreak <= -REVERSAL_THRESHOLD && !reversalState.active) {
    reversalState.active = true; reversalState.activatedAt = new Date().toISOString(); reversalState.reversalCount++;
  ...
  if (reversalState.active) {
    ...
    return { prediction: reversedPrediction, reversed: true, originalPrediction: prediction };
  }
  return { prediction, reversed: false };
}
...
function updateReversalState(type, isCorrect) {
  ...
  if (isCorrect && reversalState.active) {
    ...
    reversalState.active = false; reversalState.lastReversalResult = 'success'; reversalState.consecutiveLosses = 0;
  }
  if (!isCorrect) { reversalState.consecutiveLosses++; } else { reversalState.consecutiveLosses = 0; }
}
...
function calculateAdvancedPrediction(data, type) {
  ...
  initializePatternStats(type);
  let predictions = []; let factors = []; let allPatterns = [];
  
  const cauBet = analyzeCauBet(results, type);
  if (cauBet.detected) { predictions.push({ prediction: cauBet.prediction, confidence: cauBet.confidence, priority: 10, name: cauBet.name }); factors.push(cauBet.name); allPatterns.push(cauBet); }
  ...
  if (cyclePattern.detected) { predictions.push({ prediction: cyclePattern.prediction, confidence: cyclePattern.confidence, priority: 7, name: cyclePattern.name }); factors.push(cyclePattern.name); allPatterns.push(cyclePattern); }
  
  const cau44 = analyzeCau44(results, type);
  if (cau44.detected) { predictions.push({ prediction: cau44.prediction, confidence: cau44.confidence, priority: 9, name: cau44.name }); factors.push(cau44.name); allPatterns.push(cau44); }
  
  const cau55 = analyzeCau55(results, type);
  if (cau55.detected) { predictions.push({ prediction: cau55.prediction, confidence: cau55.confidence, priority: 9, name: cau55.name }); factors.push(cau55.name); allPatterns.push(cau55); }
  
  const cau212 = analyzeCau212(results, type);
  if (cau212.detected) { predictions.push({ prediction: cau212.prediction, confidence: cau212.confidence, priority: 8, name: cau212.name }); factors.push(cau212.name); allPatterns.push(cau212); }
  
  const cau1221 = analyzeCau1221(results, type);
  if (cau1221.detected) { predictions.push({ prediction: cau1221.prediction, confidence: cau1221.confidence, priority: 8, name: cau1221.name }); factors.push(cau1221.name); allPatterns.push(cau1221); }
  
  const cau2112 = analyzeCau2112(results, type);
  if (cau2112.detected) { predictions.push({ prediction: cau2112.prediction, confidence: cau2112.confidence, priority: 8, name: cau2112.name }); factors.push(cau2112.name); allPatterns.push(cau2112); }
  
  const cauGap = analyzeCauGap(results, type);
  if (cauGap.detected) { predictions.push({ prediction: cauGap.prediction, confidence: cauGap.confidence, priority: 7, name: cauGap.name }); factors.push(cauGap.name); allPatterns.push(cauGap); }
  
  const cauZiczac = analyzeCauZiczac(results, type);
  if (cauZiczac.detected) { predictions.push({ prediction: cauZiczac.prediction, confidence: cauZiczac.confidence, priority: 8, name: cauZiczac.name }); factors.push(cauZiczac.name); allPatterns.push(cauZiczac); }
  
  const cauDoi = analyzeCauDoi(results, type);
  if (cauDoi.detected) { predictions.push({ prediction: cauDoi.prediction, confidence: cauDoi.confidence, priority: 8, name: cauDoi.name }); factors.push(cauDoi.name); allPatterns.push(cauDoi); }
  
  const cauRong = analyzeCauRong(results, type);
  if (cauRong.detected) { predictions.push({ prediction: cauRong.prediction, confidence: cauRong.confidence, priority: 10, name: cauRong.name }); factors.push(cauRong.name); allPatterns.push(cauRong); }
  
  const smartBet = analyzeSmartBet(results, type);
  if (smartBet.detected) { predictions.push({ prediction: smartBet.prediction, confidence: smartBet.confidence, priority: 9, name: smartBet.name }); factors.push(smartBet.name); allPatterns.push(smartBet); }
  
  const diceTrendLine = analyzeDiceTrendLine(last50, type);
  if (diceTrendLine.detected) { predictions.push({ prediction: diceTrendLine.prediction, confidence: diceTrendLine.confidence, priority: 11, name: diceTrendLine.name }); factors.push(diceTrendLine.name); allPatterns.push(diceTrendLine); }
  
  const breakPattern = analyzeBreakPattern(results, last50, type);
  if (breakPattern.detected) { predictions.push({ prediction: breakPattern.prediction, confidence: breakPattern.confidence, priority: 12, name: breakPattern.name }); factors.push(breakPattern.name); allPatterns.push(breakPattern); }
  
  const dayGay = analyzeDayGay(last50, type);
  if (dayGay.detected) { predictions.push({ prediction: dayGay.prediction, confidence: dayGay.confidence, priority: 13, name: dayGay.name }); factors.push(dayGay.name); allPatterns.push(dayGay); }
  ...
  if (distribution.imbalance > 0.2) {
    ...
    predictions.push({ prediction: minority, confidence: Math.round(6 * weight), priority: 5, name: 'Phân bố lệch' }); factors.push(`Phân bố lệch (T:${distribution.taiPercent.toFixed(0)}% - X:${distribution.xiuPercent.toFixed(0)}%)`);
  }
  ...
  if (dicePatterns.averageSum > 11.5) {
    ...
    predictions.push({ prediction: 'Xỉu', confidence: Math.round(5 * weight), priority: 4, name: 'Tổng TB cao' }); factors.push(`Tổng TB cao (${dicePatterns.averageSum.toFixed(1)})`);
  } else if (dicePatterns.averageSum < 9.5) {
    const weight = getPatternWeight(type, 'dice_pattern');
    predictions.push({ prediction: 'Tài', confidence: Math.round(5 * weight), priority: 4, name: 'Tổng TB thấp' }); factors.push(`Tổng TB thấp (${dicePatterns.averageSum.toFixed(1)})`);
  }
  ...
  if (sumTrend.strength > 0.4) {
    ...
    predictions.push({ prediction: trendPrediction, confidence: Math.round(4 * weight), priority: 3, name: 'Xu hướng tổng' }); factors.push(`Xu hướng tổng ${sumTrend.trend === 'increasing' ? 'tăng' : 'giảm'}`);
  }
  
  const edgeCases = analyzeEdgeCases(last50, type);
  if (edgeCases.detected) { predictions.push({ prediction: edgeCases.prediction, confidence: edgeCases.confidence, priority: 5, name: edgeCases.name }); factors.push(edgeCases.name); allPatterns.push(edgeCases); }
  ...
  if (momentum.window_3 && momentum.window_10) {
    ...
    if (shortTermDiff > 0.3) {
      ...
      const weight = getPatternWeight(type, 'momentum');
      predictions.push({ prediction: reversePrediction, confidence: Math.round(5 * weight), priority: 4, name: 'Biến động ngắn hạn' }); factors.push('Biến động ngắn hạn mạnh');
    }
  }
  
  const fibonacciPattern = analyzeFibonacciPattern(last50, type);
  if (fibonacciPattern.detected) { predictions.push({ prediction: fibonacciPattern.prediction, confidence: fibonacciPattern.confidence, priority: 8, name: fibonacciPattern.name }); factors.push(fibonacciPattern.name); allPatterns.push(fibonacciPattern); }
  
  const momentumPattern = analyzeMomentumPattern(last50, type);
  if (momentumPattern.detected) { predictions.push({ prediction: momentumPattern.prediction, confidence: momentumPattern.confidence, priority: 9, name: momentumPattern.name }); factors.push(momentumPattern.name); allPatterns.push(momentumPattern); }
  
  const resistanceSupport = analyzeResistanceSupport(last50, type);
  if (resistanceSupport.detected) { predictions.push({ prediction: resistanceSupport.prediction, confidence: resistanceSupport.confidence, priority: 10, name: resistanceSupport.name }); factors.push(resistanceSupport.name); allPatterns.push(resistanceSupport); }
  
  const wavePattern = analyzeWavePattern(last50, type);
  if (wavePattern.detected) { predictions.push({ prediction: wavePattern.prediction, confidence: wavePattern.confidence, priority: 8, name: wavePattern.name }); factors.push(wavePattern.name); allPatterns.push(wavePattern); }
  
  const goldenRatio = analyzeGoldenRatio(last50, type);
  if (goldenRatio.detected) { predictions.push({ prediction: goldenRatio.prediction, confidence: goldenRatio.confidence, priority: 9, name: goldenRatio.name }); factors.push(goldenRatio.name); allPatterns.push(goldenRatio); }
  
  const markovChain = analyzeMarkovChain(results, last50, type);
  if (markovChain.detected) { predictions.push({ prediction: markovChain.prediction, confidence: markovChain.confidence, priority: 12, name: markovChain.name }); factors.push(markovChain.name); allPatterns.push(markovChain); }
  
  const movingAvgDrift = analyzeMovingAverageDrift(last50, type);
  if (movingAvgDrift.detected) { predictions.push({ prediction: movingAvgDrift.prediction, confidence: movingAvgDrift.confidence, priority: 11, name: movingAvgDrift.name }); factors.push(movingAvgDrift.name); allPatterns.push(movingAvgDrift); }
  
  const sumPressure = analyzeSumPressure(last50, type);
  if (sumPressure.detected) { predictions.push({ prediction: sumPressure.prediction, confidence: sumPressure.confidence, priority: 11, name: sumPressure.name }); factors.push(sumPressure.name); allPatterns.push(sumPressure); }
  
  const volatilityPattern = analyzeVolatility(last50, type);
  if (volatilityPattern.detected) { predictions.push({ prediction: volatilityPattern.prediction, confidence: volatilityPattern.confidence, priority: 10, name: volatilityPattern.name }); factors.push(volatilityPattern.name); allPatterns.push(volatilityPattern); }
  
  const sunHotCold = analyzeSunHotCold(results, last50, type);
  if (sunHotCold.detected) { predictions.push({ prediction: sunHotCold.prediction, confidence: sunHotCold.confidence, priority: 13, name: sunHotCold.name }); factors.push(sunHotCold.name); allPatterns.push(sunHotCold); }
  
  const sunStreakBreak = analyzeSunStreakBreak(results, last50, type);
  if (sunStreakBreak.detected) { predictions.push({ prediction: sunStreakBreak.prediction, confidence: sunStreakBreak.confidence, priority: 14, name: sunStreakBreak.name }); factors.push(sunStreakBreak.name); allPatterns.push(sunStreakBreak); }
  
  const sunBalance = analyzeSunBalance(results, type);
  if (sunBalance.detected) { predictions.push({ prediction: sunBalance.prediction, confidence: sunBalance.confidence, priority: 12, name: sunBalance.name }); factors.push(sunBalance.name); allPatterns.push(sunBalance); }
  
  const sunMomentumShift = analyzeSunMomentumShift(results, last50, type);
  if (sunMomentumShift.detected) { predictions.push({ prediction: sunMomentumShift.prediction, confidence: sunMomentumShift.confidence, priority: 13, name: sunMomentumShift.name }); factors.push(sunMomentumShift.name); allPatterns.push(sunMomentumShift); }
  ...
  return {
    ...
    detailedAnalysis: {
      ...
      distribution, dicePatterns, sumTrend, adaptiveBoost,
...
// FIX & NÂNG CẤP: Thay đổi định dạng xuất ra đúng chuẩn yêu cầu
app.get('/sun', async (req, res) => {
  try {
    ...
    // Non-blocking: Kiểm tra kết quả ngầm (không để User phải chờ lâu)
    setTimeout(() => {
      verifyPredictions('b52', data.data).catch(console.error);
    }, 0);
    
    const sunData = data.data;
    const prevRound = sunData[0];
    const latestPhien = prevRound.Phien;
    const nextPhien = latestPhien + 1;
    
    const result = calculateAdvancedPrediction(sunData, 'b52');
    
    // Ghi log ngầm (không block request)
    setTimeout(() => {
      savePredictionToHistory('b52', nextPhien, result.prediction, result.confidence);
      recordPrediction('b52', nextPhien, result.prediction, result.confidence, result.factors);
    }, 0);
    
    // Dịch % độ tin cậy sang chữ viết tắt
    let do_tin_cay_text = "Thấp";
    if (result.confidence >= 80) do_tin_cay_text = "Rất";
    else if (result.confidence >= 70) do_tin_cay_text = "Mạnh";
    else if (result.confidence >= 60) do_tin_cay_text = "Khá";
    
    // Định dạng chuỗi kết quả: Tài'3_5_4 hoặc Xỉu'1_2_3
    const ket_qua_format = `${prevRound.Ket_qua}'${prevRound.xuc_xac_1}_${prevRound.xuc_xac_2}_${prevRound.xuc_xac_3}`;
    
    // Trả về JSON theo đúng định dạng được yêu cầu
    res.json({
      phien: prevRound.Phien,
      ket_qua: ket_qua_format,
      phien_hien_tai: nextPhien,
      du_doan: result.prediction,
      do_tin_cay: do_tin_cay_text
    });
...
app.get('/sun/lichsu', async (req, res) => {
  try {
    ...
    if (data && data.data) {
      // Non-blocking verify
      verifyPredictions('b52', data.data).catch(e => {});
    }
    ...
    const historyWithStatus = predictionHistory.b52.map(record => {
      ...
      let status = null; let ket_qua_thuc_te = null;
      if (prediction && prediction.verified) {
        status = prediction.isCorrect ? '✅' : '❌';
        ket_qua_thuc_te = prediction.actual;
      }
      return { ...record, ket_qua_thuc_te, status };
    });
    
    res.json({ type: 'Sun Tài Xỉu', history: historyWithStatus, total: historyWithStatus.length });
  } catch (error) {
    res.json({ type: 'Sun Tài Xỉu', history: predictionHistory.b52, total: predictionHistory.b52.length, error: 'Không thể cập nhật trạng thái' });
  }
});
  ...
  const stats = {
    sun: {
      ...
      accuracy: learningData.b52.totalPredictions > 0 
        ? (learningData.b52.correctPredictions / learningData.b52.totalPredictions * 100).toFixed(2) + '%' : 'N/A',
      ...
      autoReversal: {
        active: reversalState.active, activatedAt: reversalState.activatedAt,
        totalReversals: reversalState.reversalCount, consecutiveLosses: reversalState.consecutiveLosses,
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  GET /sun - Get prediction for next Sun round (Fixed & Non-blocking)');
